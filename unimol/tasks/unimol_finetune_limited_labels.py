...
@register_task("mol_finetune_limited_labels")
class UniMolFinetuneLimitedLabelsTask(UnicoreTask):
    ...
    def load_dataset(self, split, **kwargs):
        ...
        # Drop NaN values
        dataset._keys = [dataset._keys[i] for i in range(len(dataset._keys)) if not np.isnan(dataset[i]['target'][0])]
        # Random shhuffle dataset and select the subset
        np.random.seed(self.args.seed)
        dataset._keys = np.random.permutation(dataset._keys)
        
        if split == "train":
            dataset._keys = dataset._keys[:self.args.max_num_labels]
