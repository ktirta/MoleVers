...
@register_loss("unimol_drl_noisycoords")
class UniMolLossDRL_noisycoords(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        # Diffusion Loss
        dist_noise = diffusion_dict["coord_noise"].float()
        dist_pred = diffusion_dict["coord_pred"].float()

        noisycoords_loss = self.args.diffusion_loss * (F.mse_loss(dist_noise, dist_pred, reduction="mean"))

        loss = loss + noisycoords_loss
        logging_output["noisycoords_loss"] = noisycoords_loss.data
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        noisycoords_loss = sum(log.get("noisycoords_loss", 0) for log in logging_outputs)
        metrics.log_scalar(
            "noisycoords_loss", noisycoords_loss / sample_size, sample_size, round=3
        )
...
@register_loss("unimol_infer_drl_noisycoords")
class UniMolInferLossDRL_noisycoords(UnicoreLoss):
    def __init__(self, task):
        super().__init__(task)
        self.padding_idx = task.dictionary.pad()
