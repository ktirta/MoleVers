...
@register_loss("unimol_drl_noisy_atoms_and_coords")
class UniMolLossDRL_noisy_atoms_and_coords(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if (masked_tokens is not None) and (logits_encoder is not None):
        ...
        # Diffusion Loss
        coord_noise = diffusion_dict["coord_noise"].float()
        coord_pred = diffusion_dict["coord_pred"].float()
        coord_diff_loss = self.args.diffusion_loss * F.mse_loss(coord_noise, coord_pred, reduction="mean")

        dist_noise = diffusion_dict["dist_noise"].float()
        dist_pred = diffusion_dict["dist_pred"].float()
        dist_diff_loss = self.args.diffusion_loss * F.mse_loss(dist_noise, dist_pred, reduction="mean")

        atom_noise = diffusion_dict["atom_noise"].float()
        atom_pred = diffusion_dict["atom_pred"].float()
        atom_diff_loss = self.args.diffusion_loss * F.mse_loss(atom_noise, atom_pred, reduction="mean")


        loss = loss + coord_diff_loss + dist_diff_loss + atom_diff_loss

        logging_output["coord_diff_loss"] = coord_diff_loss.data
        logging_output["dist_diff_loss"] = dist_diff_loss.data
        logging_output["atom_diff_loss"] = atom_diff_loss.data
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        diff_coord_loss = sum(log.get("coord_diff_loss", 0) for log in logging_outputs)
        diff_atom_loss = sum(log.get("atom_diff_loss", 0) for log in logging_outputs)
        diff_dist_loss = sum(log.get("dist_diff_loss", 0) for log in logging_outputs)

        metrics.log_scalar(
            "diff_coord_loss", diff_coord_loss / sample_size, sample_size, round=3
        )
        metrics.log_scalar(
            "diff_atom_loss", diff_atom_loss / sample_size, sample_size, round=3
        )
        metrics.log_scalar(
            "diff_dist_loss", diff_dist_loss / sample_size, sample_size, round=3
        )
...
@register_loss("unimol_infer_drl_noisy_atoms_and_coords")
class UniMolInferLossDRL_noisy_atoms_and_coords(UnicoreLoss):
    def __init__(self, task):
        super().__init__(task)
        self.padding_idx = task.dictionary.pad()
