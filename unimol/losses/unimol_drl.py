...
@register_loss("unimol_drl")
class UniMolLossDRL(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        # Diffusion Loss
        x_noise = diffusion_dict["x_noise"].float()
        x_pred = diffusion_dict["x_pred"].float()
        dist_noise = diffusion_dict["dist_noise"].float()
        dist_pred = diffusion_dict["dist_pred"].float()

        diffusion_loss = self.args.diffusion_loss * (F.mse_loss(x_pred, x_noise, reduction="mean") + F.mse_loss(dist_noise, dist_pred, reduction="mean"))

        loss = loss + diffusion_loss
        logging_output["diffusion_loss"] = diffusion_loss.data
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        diffusion_loss = sum(log.get("diffusion_loss", 0) for log in logging_outputs)
        metrics.log_scalar(
            "diffusion_loss", diffusion_loss / sample_size, sample_size, round=3
        )
...
@register_loss("unimol_infer_drl")
class UniMolInferLossDRL(UnicoreLoss):
    def __init__(self, task):
        super().__init__(task)
        self.padding_idx = task.dictionary.pad()
