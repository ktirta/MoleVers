...
@register_loss("multi_task_BCE_ranking")
class MultiTaskBCERankingLoss(CrossEntropyLoss):
    def __init__(self, task):
        super().__init__(task)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        logit_output = model.classification_heads['multi-ranking'](mol1_rep, mol2_rep)   
        logit_output = logit_output.squeeze(-1)   

        is_valid = sample["target"]["finetune_target"] > -0.5 # sanity check
        loss = self.compute_loss(
            model, logit_output, sample, reduce=reduce, is_valid=is_valid
        )
        sample_size = sample["target"]["finetune_target"].size(0)
...
@register_loss("multi_task_BCE_ranking_regression")
class MultiTaskBCERankingRegressionLoss(CrossEntropyLoss):
    def __init__(self, task):
        super().__init__(task)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        output_dic = model.classification_heads['multi-ranking-regression'](mol1_rep, mol2_rep)   
        logit_output = output_dic['cls_logits']
        reg1_output = output_dic['reg1']
        reg2_output = output_dic['reg2']
        logit_output = logit_output.squeeze(-1)   
        is_valid = sample["target"]["finetune_target"] > -0.5 # sanity check

        loss, cls_loss, reg_loss = self.compute_loss(
            model, logit_output, reg1_output, reg2_output, sample, reduce=reduce, is_valid=is_valid
        )
        sample_size = sample["target"]["finetune_target"].size(0)
        if not self.training:
            probs = torch.sigmoid(logit_output.float()).view(-1, logit_output.size(-1))
    ...
    def compute_loss(self, model, cls_output, reg1_output, reg2_output, sample, reduce=True, is_valid=None):
        pred = cls_output[is_valid].float()
        targets = sample["target"]["finetune_target"][is_valid].float()
        reg1_target = sample["target"]["reg1_target"].float()
        reg2_target = sample["target"]["reg2_target"].float()
        ...
        reg_loss = F.l1_loss(torch.cat((reg1_output, reg2_output), dim=1), 
                                torch.cat((reg1_target, reg2_target), dim=1))
    ...
    def reduce_metrics(logging_outputs, split="valid") -> None:
        ...
        metrics.log_scalar(
            "cls_loss", sum(log.get("cls_loss", 0) for log in logging_outputs) / sample_size / math.log(2), sample_size, round=3
        )
        metrics.log_scalar(
